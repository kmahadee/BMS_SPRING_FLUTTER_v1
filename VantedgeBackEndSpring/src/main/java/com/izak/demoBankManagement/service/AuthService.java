...
//    }
    ...
    //========================================================================
    //                   OLD LOGIN METHOD WITHOUT STATUS CHECK
    //========================================================================
//    @Transactional
//    public AuthResponseDTO login(LoginRequestDTO request) {
//        try {
...
//
    ...
    //========================================================================
    //                   NEW LOGIN METHOD WITH STATUS CHECK
    //========================================================================
    ...
    public AuthResponseDTO login(LoginRequestDTO request) {
        try {
            ...
            // ============================================================================
            // NEW: User Status Validation
            // ============================================================================
            // Validate user status before allowing login
            // Treat null userStatus as ACTIVE for backward compatibility
            if (user.getUserStatus() != null) {
                ...
                if (user.getUserStatus() == User.UserStatus.PENDING) {
                    log.warn("Login attempt by pending user: {}", user.getUsername());
                    throw new InvalidCredentialsException(
                            "Account is pending approval. Please wait for administrator approval.");
                }
            }
            // ============================================================================
            // End of User Status Validation
            // ============================================================================
            ...
            // Add branchId for branch-specific roles (excluding ADMIN and CUSTOMER)
            if (user.getBranch() != null &&
                    user.getRole() != User.Role.ADMIN &&
            ...
            // Re-throw our custom exceptions with specific messages
            log.error("Login failed for user: {} - Reason: {}", request.getUsername(), e.getMessage());
            throw e;
        } catch (Exception e) {
...
//    }
    ...
    public AuthResponseDTO refreshToken(RefreshTokenRequestDTO request) {
        ...
        if (jwtUtil.validateToken(request.getRefreshToken(), userDetails)) {
            ...
            if (user.getUserStatus() != null) {
                if (user.getUserStatus() == User.UserStatus.INACTIVE) {
                    log.warn("Token refresh attempt by inactive user: {}", user.getUsername());
                    throw new InvalidCredentialsException(
                            "Account is inactive. Please contact administrator.");
                }

                if (user.getUserStatus() == User.UserStatus.PENDING) {
                    log.warn("Token refresh attempt by pending user: {}", user.getUsername());
                    throw new InvalidCredentialsException(
                            "Account is pending approval. Please wait for administrator approval.");
                }
            }
